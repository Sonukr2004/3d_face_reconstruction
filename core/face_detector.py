...
Thread-safe: each thread gets its own CascadeClassifier instance via
threading.local(), preventing C++ vector::_M_range_check crashes under
...
class FaceDetector:
    ...
    def _detector(self) -> cv2.CascadeClassifier:
        """Return a per-thread CascadeClassifier instance."""
        det = getattr(self._tls, "detector", None)
        if det is None:
            det = cv2.CascadeClassifier(self._cascade_path)
